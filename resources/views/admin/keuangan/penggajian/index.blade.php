...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const historiPenggajianDiv = document.getElementById('histori_penggajian');

    // Handle tim and jabatan selection
    timSelect.addEventListener('change', updateEmployees);
    jabatanSelect.addEventListener('change', updateEmployees);

    function updateEmployees() {
        const timId = timSelect.value;
        const jabatanId = jabatanSelect.value;
    ...
    employeeSelect.addEventListener('change', function () {
        ...
        if (employeeId) {
            $.ajax({
                ...
                success: function (response) {
                    tanggalMasukInput.value = response.tanggal_masuk;
                    noRekeningInput.value = response.no_rekening;
                    namaBankInput.value = response.nama_bank;

                    const histori = response.histori_penggajian;
                    let historiHtml = '';
                    if (histori.length > 0) {
                        historiHtml += '<ul>';
                        histori.forEach(function (gaji) {
                    ...
                    historiPenggajianDiv.innerHTML = historiHtml;
                }
            });
        }
    });
